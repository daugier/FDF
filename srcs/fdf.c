...
/*   Updated: 2016/06/27 01:01:34 by daugier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		define_y(t_struct *data, int x, int y)
{
	...
	{
		if (Z > 100)
			COLOR = mlx_get_color_value(MLX, 0x990000);
		if (Z <= 100 && Z > 40)
			COLOR = mlx_get_color_value(MLX, 0xCC0033);
		if (Z <= 40 && Z > 0)
			COLOR = mlx_get_color_value(MLX, 0xFF6600);
		if (Z <= 0 && Z > -20)
			COLOR = mlx_get_color_value(MLX, 0x33CC00);
		if (Z <= -20)
